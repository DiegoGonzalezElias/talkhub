import { collection, query, where, getDocs, getDoc, doc, updateDoc, arrayUnion, setDoc, onSnapshot } from 'firebase/firestore';
...
export const getMessages = async (updateMessages) => {
  try {
    const userStore = useUserStore();
    const $toast = useToast();
    const existingselectContact = JSON.parse(localStorage.getItem('selectedContact'));
